      ...
      <div className="max-w-6xl mx-auto">
              ...
              <Button
                ...
                className={`transition-all duration-300 ${
                  selectedCategory === category 
                    ? "bg-gradient-primary hover:shadow-glow-primary scale-105" 
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6 mb-16">
          {stats.map((stat, index) => (
            <Card 
              key={index} 
              className="bg-card/50 border border-border/80 hover:border-primary/60 transition-all duration-300 hover:shadow-glow-primary/20 text-center group animate-fade-in"
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {filteredAchievements.map((achievement, index) => (
            <Card 
              key={index} 
              className="bg-card/50 border border-border/80 hover:border-primary/60 transition-all duration-500 hover:shadow-glow-primary/20 group animate-fade-in overflow-hidden"
              style={{ animationDelay: `${index * 0.1}s` }}
              ...
              <CardContent className="p-6 relative">
                ...
                <div className="absolute top-2 right-2">
                  <div className={`w-3 h-3 rounded-full ${
                    achievement.status === 'completed' ? 'bg-success' : 'bg-warning animate-pulse'
                  }`} />
                </div>
                  ...
                  <div className="flex flex-wrap gap-1">
                    {achievement.techStack.map((tech, techIndex) => (
                      <Badge 
                        key={techIndex}
                        variant="outline"
                        className="text-xs border-primary/20 text-primary/80 hover:bg-primary/10 transition-colors duration-200"
                      >
                        {tech}
                      </Badge>
                    ))}
        ...
        <div className="text-center mt-16 space-y-6">
          <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-secondary/50 border border-border/50">
            ...
            <span className="text-sm text-muted-foreground">Continuously achieving new milestones</span>
          </div>
